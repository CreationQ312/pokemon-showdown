...
export const Moves: import("../sim/dex-moves").MoveDataTable = {
	...
	beatup: {
		...
		basePower: 0,
		basePowerCallback(pokemon, target, move) {
			const setSpecies = this.dex.species.get(
				move.allies!.shift()!.set.species
			);
...
//		heal: [1, 2],
	...
	chillingvenom: {
		...
		//immunity bypass implemented in sim/pokemon.ts
		target: "normal",
		type: "Ice",
	},
	chillingwater: {
	...
	conversion2: {
		...
		onHit(target, source) {
			...
			for (const typeName of this.dex.types.names()) {
				...
				const typeCheck =
					this.dex.types.get(typeName).damageTaken[attackType];
	...
	crushgrip: {
		...
		basePowerCallback(pokemon, target) {
			...
			const bp =
				Math.floor(
					Math.floor(
						(120 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) /
							4096
					) / 100
				) || 1;
	...
	dragonclutch: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, mirror: 1, protect: 1, metronome: 1 },
		secondary: {
			chance: 30,
			boosts: {
				def: -1,
	...
	drillpeck: {
		...
		pp: 20,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, distance: 1, metronome: 1, spin: 1 },
		target: "any",
		type: "Flying",
		contestType: "Cool",
	},
	...
	echoedvoice: {
		...
		basePowerCallback(pokemon, target, move) {
			...
			if (this.field.pseudoWeather.echoedvoice) {
				bp =
					move.basePower * this.field.pseudoWeather.echoedvoice.multiplier;
			}
	...
	eonrift: {
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (!target || target.fainted || target.hp <= 0) {
				if (
					pokemon.getStat("spe", false, true) >
					pokemon.getStat("spa", false, true)
				) {
					this.hint(
						"Eon Rift made electric current flow through the battlefield!"
					);
					this.field.setTerrain("electricterrain");
				} else if (
					pokemon.getStat("spe", false, true) <=
	...
	etherburst: {
		...
		onModifyType(move, pokemon, target) {
			...
			this.hint(
				`Ether Burst changed its type to: ${move.type} to match ${pokemon.name}'s primary typing`
			);
		},
		onModifyMove(move, pokemon) {
			if (
	...
	flail: {
		...
		basePowerCallback(pokemon) {
			const ratio = Math.max(
				Math.floor((pokemon.hp * 48) / pokemon.maxhp),
				1
			);
			let bp;
			if (ratio < 2) {
	...
	flamewheel: {
		...
		pp: 25,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, defrost: 1, metronome: 1, spin: 1 },
		secondary: {
			chance: 10,
			status: 'brn',
		},
	...
	gyroball: {
		...
		pp: 5,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, bullet: 1, spin: 1 },
		target: "normal",
		type: "Steel",
		zMove: { basePower: 160 },
		maxMove: { basePower: 130 },
	...
	hardpress: {
		...
		basePowerCallback(pokemon, target) {
			...
			const bp =
				Math.floor(
					Math.floor(
						(100 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) /
							4096
					) / 100
				) || 1;
			this.debug(`BP for ${hp}/${maxHP} HP: ${bp}`);
	...
	healblock: {
		...
		condition: {
			...
			durationCallback(target, source, effect) {
				if (
					effect?.name === "Psychic Noise" ||
					effect?.name === "Wild Wire"
				) {
					return 2;
				}
				if (source?.hasAbility("persistent")) {
			...
			onTryHeal(damage, target, source, effect) {
				if (effect && (effect.id === "zpower" || (effect as Move).isZ))
					return damage;
				if (
					source &&
					target !== source &&
	...
	heatsiphon: {
		...
		onHit(target, source) {
			if (["", "slp", "frz", "par", "psn", "tox"].includes(target.status)) {
				this.hint(
					"Heat Siphon only works if the targeted Pokemon is burned."
				);
				return false;
			}
			target.cureStatus();
			...
			onTryHit(target, source, move) {
				...
				if (this.checkMoveMakesContact(move, source, target)) {
					this.boost(
						...
						source,
						target,
						this.dex.getActiveMove("Ironclad Guard")
					);
				}
				return this.NOT_FAIL;
			},
	...
	relicsong: {
		...
		onHit(target, pokemon, move) {
			if (
				pokemon.species.id === "meloetta" ||
				(pokemon.species.id === "meloettapirouette" && !pokemon.transformed)
			) {
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (
				(move.willChangeForme && pokemon.species.id === "meloetta") ||
				pokemon.species.id === "meloettapirouette"
			) {
				const meloettaForme =
	...
	shellsidearm: {
		...
		onModifyMove(move, pokemon, target) {
			...
			if (
				physical > special ||
				(physical === special && this.randomChance(1, 2))
			) {
				move.category = "Physical";
	...
	steamroller: {
		...
		pp: 20,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, spin: 1, minimize: 1 },
		secondary: {
			chance: 30,
			volatileStatus: "flinch",
		},
	...
	stomp: {
		...
		pp: 20,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, nonsky: 1, metronome: 1, kick: 1, minimize: 1 },
		secondary: {
			chance: 30,
			volatileStatus: "flinch",
		},
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (!target || target.fainted || target.hp <= 0) {
				if (
					...
					this.hint(
						"Timeless Torrent absorbed the Power and Speed of its fallen target!"
					);
				} else if (this.field.isWeather("sunnyday")) {
					this.boost({ spa: 1 }, pokemon, pokemon, move);
					//this.add('-activate', pokemon, 'move: Timeless Torrent', "[inSun]");
					this.hint(
						"Timeless Torrent absorbed the Power of its fallen target!"
					);
					...
					this.hint(
						"Timeless Torrent absorbed the Speed of its fallen target!"
					);
				} else {
					this.heal(pokemon.baseMaxhp / 10, pokemon, pokemon, move);
					this.hint(
						"Timeless Torrent absorbed the Health of its fallen target!"
					);
	...
	tribeam: {
		...
		onPrepareHit(target, source, move) {
			...
			if (vsElectric > vsFire && vsElectric > vsIce) {
				...
				if (
					this.field.isWeather("sunnyday") ||
					this.field.isWeather("desolateland")
				) {
					move.type = "Fire";
				} else if (this.field.isTerrain("electricterrain")) {
					move.type = "Electric";
			...
			this.hint(
				`Tri Beam changed its type to: ${move.type} to maximize damage against ${target.name}.`
			);
		},
	...
	wormholerush: { //wow its like exalted brew but AGAIN
		//delta source code CANNOT leak faster
		//
		//more like aegislash delta move but again ;]
		num: -110, 
		accuracy: true,
		basePower: 0,
		category: "Status",
...
//		forceSwitch: true,
		onModifyMove(move, pokemon, target) {
			if (pokemon.species.name === "Slakoth-ATOM") {
				move.basePower = 180;
				move.target = "normal";
				move.category = "Physical";	
	...
	pulsarspiral: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, contact: 1, spin: 1 },
		basePowerCallback(pokemon, target, move) {
			if(pokemon.ability === "Spinurgy" && pokemon.abilityState.spinurgy === 3){
				move.basePower = 100;
			} 
			return move.basePower;
		},
	...
	titanbuster: {
		...
		pp: 5,
		priority: 0,
		flags: { charge: 1, protect: 1, mirror: 1, contact: 1, failinstruct: 1, nosleeptalk: 1, metronome: 1 },
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
			}
